# thedailystoke — THE PACKET tournament, operator run
...
| Path | What it is |
|---|---|
| `blueprint/BLUEPRINT-FINAL.md` | The converged blueprint: mind, platform, charter, economics, prevention table, proof plan, limits, real-world gates |
| `blueprint/CHARTER-DRAFT.md` | Draft legal text (trust deed, certificate, bylaws, privacy covenant), v2 — **not legal advice, draft for counsel review** |
| `blueprint/BLUEPRINT-v1…v4.md` | Each synthesis round, with scores, merge/reject decisions and change lists |
| `reference-mind/` | Executable spec of the personal agent (stdlib Python; `python3 -m unittest discover -s tests -t .`, `python3 demo.py`) |
| `LEDGER.md` | 17-dimension scores per run, blind-spot trendline, all-time records |
| `run-1/`, `run-2/`, `run-3/` | Every phase output (01 reckoning → 05 red-teamed final design), each run's mind, funding models, and `RUN-CARD.md` |
| `run-3/heldout/` | Orchestrator-written held-out loyalty battery (withheld from builders) and results |
